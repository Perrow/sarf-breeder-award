from django.contrib import admin
from django.contrib.auth import get_user_model
from django.db import IntegrityError, transaction
from django.test import RequestFactory, TestCase
from django.urls import reverse

from progression.models import DeMeritBadge, UserDeMeritBadge


class DeMeritBadgeTests(TestCase):
    def setUp(self):
        user_model = get_user_model()
        self.user = user_model.objects.create_user(
            username="demerit-user",
            email="demerit-user@example.com",
            password="Test-password-123",
        )
        self.other_user = user_model.objects.create_user(
            username="demerit-other",
            email="demerit-other@example.com",
            password="Test-password-123",
        )
        self.staff = user_model.objects.create_user(
            username="demerit-staff",
            email="demerit-staff@example.com",
            password="Test-password-123",
            is_staff=True,
        )
        self.superuser = user_model.objects.create_superuser(
            username="demerit-admin",
            email="demerit-admin@example.com",
            password="Test-password-123",
        )
        self.badge = DeMeritBadge.objects.create(
            name="Glömde doppvärmaren",
            description="För ett minnesvärt temperaturmisstag.",
        )

    def test_same_badge_can_only_be_awarded_once_per_user(self):
        UserDeMeritBadge.objects.create(user=self.user, badge=self.badge)

        with self.assertRaises(IntegrityError):
            with transaction.atomic():
                UserDeMeritBadge.objects.create(user=self.user, badge=self.badge)

    def test_different_users_can_award_the_same_badge(self):
        UserDeMeritBadge.objects.create(user=self.user, badge=self.badge)
        UserDeMeritBadge.objects.create(user=self.other_user, badge=self.badge)

        self.assertEqual(UserDeMeritBadge.objects.filter(badge=self.badge).count(), 2)

    def test_only_superuser_can_administer_badge_catalog(self):
        badge_admin = admin.site._registry[DeMeritBadge]
        factory = RequestFactory()

        staff_request = factory.get("/admin/")
        staff_request.user = self.staff
        superuser_request = factory.get("/admin/")
        superuser_request.user = self.superuser

        self.assertFalse(badge_admin.has_module_permission(staff_request))
        self.assertFalse(badge_admin.has_add_permission(staff_request))
        self.assertFalse(badge_admin.has_change_permission(staff_request))
        self.assertFalse(badge_admin.has_delete_permission(staff_request))
        self.assertTrue(badge_admin.has_module_permission(superuser_request))
        self.assertTrue(badge_admin.has_add_permission(superuser_request))
        self.assertTrue(badge_admin.has_change_permission(superuser_request))
        self.assertTrue(badge_admin.has_delete_permission(superuser_request))

    def test_logged_in_user_can_self_award_active_badge(self):
        self.client.force_login(self.user)

        response = self.client.post(reverse("award_demerit_badge", args=[self.badge.pk]))

        self.assertRedirects(response, reverse("demerit_badges"))
        self.assertTrue(
            UserDeMeritBadge.objects.filter(user=self.user, badge=self.badge).exists()
        )

    def test_self_award_endpoint_ignores_attempted_other_user(self):
        self.client.force_login(self.user)

        self.client.post(
            reverse("award_demerit_badge", args=[self.badge.pk]),
            {"user": self.other_user.pk},
        )

        self.assertTrue(
            UserDeMeritBadge.objects.filter(user=self.user, badge=self.badge).exists()
        )
        self.assertFalse(
            UserDeMeritBadge.objects.filter(user=self.other_user, badge=self.badge).exists()
        )

    def test_awarding_same_badge_again_does_not_create_duplicate(self):
        self.client.force_login(self.user)
        url = reverse("award_demerit_badge", args=[self.badge.pk])

        self.client.post(url)
        self.client.post(url)

        self.assertEqual(
            UserDeMeritBadge.objects.filter(user=self.user, badge=self.badge).count(),
            1,
        )

    def test_inactive_badge_cannot_be_self_awarded(self):
        self.badge.active = False
        self.badge.save(update_fields=["active"])
        self.client.force_login(self.user)

        response = self.client.post(reverse("award_demerit_badge", args=[self.badge.pk]))

        self.assertEqual(response.status_code, 404)
        self.assertFalse(
            UserDeMeritBadge.objects.filter(user=self.user, badge=self.badge).exists()
        )

    def test_catalog_only_lists_active_badges(self):
        DeMeritBadge.objects.create(
            name="Inaktivt märke",
            description="Ska inte kunna väljas.",
            active=False,
        )
        self.client.force_login(self.user)

        response = self.client.get(reverse("demerit_badges"))

        self.assertContains(response, "Glömde doppvärmaren")
        self.assertNotContains(response, "Inaktivt märke")

    def test_awarded_badge_is_visible_among_achievements(self):
        UserDeMeritBadge.objects.create(user=self.user, badge=self.badge)
        self.client.force_login(self.user)

        response = self.client.get(reverse("achievements"))

        self.assertContains(response, "De-merit badges")
        self.assertContains(response, "Glömde doppvärmaren")
        self.assertContains(response, "De-merit badge · självtilldelad")
        self.assertContains(response, 'class="flex-shrink-0 text-center"')

    def test_awarded_badge_is_in_shared_award_list_on_my_page(self):
        UserDeMeritBadge.objects.create(user=self.user, badge=self.badge)
        self.client.force_login(self.user)

        response = self.client.get(reverse("breeding_list"))

        self.assertContains(response, '<h2 class="h4">Utmärkelser</h2>', html=True)
        self.assertContains(response, "Glömde doppvärmaren")
        self.assertNotContains(response, '<h2 class="h4">De-merit badges</h2>', html=True)

    def test_inactive_awarded_badge_remains_visible_among_achievements(self):
        UserDeMeritBadge.objects.create(user=self.user, badge=self.badge)
        self.badge.active = False
        self.badge.save(update_fields=["active"])
        self.client.force_login(self.user)

        response = self.client.get(reverse("achievements"))

        self.assertContains(response, "Glömde doppvärmaren")

    def test_anonymous_user_must_log_in_before_self_awarding(self):
        response = self.client.post(reverse("award_demerit_badge", args=[self.badge.pk]))

        self.assertEqual(response.status_code, 302)
        self.assertIn(reverse("login"), response.url)
